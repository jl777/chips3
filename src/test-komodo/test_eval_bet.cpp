...
int CCSign(CMutableTransaction &tx, unsigned int nIn, CC *cond, std::vector<int> keyIds) {
    ...
    uint256 sighash = SignatureHash(CCPubKey(cond), tx, nIn, SIGHASH_ALL, 0, SigVersion::BASE, &txdata);
    int nSigned = 0;
...
static bool CCVerify(const CMutableTransaction &mtxTo, const CC *cond) {
    ...
    return VerifyScript(CCSig(cond), CCPubKey(cond), NULL, 0, checker);
};
...
int TestCC(CMutableTransaction &mtxTo, unsigned int nIn, CC *cond)
{
    ...
    return VerifyScript(txTo.vin[nIn].scriptSig, CCPubKey(cond), NULL, 0, checker);
}
...
public:
    uint256 MoM;
    int currentHeight;
    std::map<uint256, CTransactionRef> txs;
    std::map<uint256, CBlockIndex> blocks;
    std::map<uint256, std::vector<CTransactionRef>> spends;
    ...
    bool GetSpendsConfirmed(uint256 hash, std::vector<CTransactionRef> &spendsOut) const
    {
    ...
    bool GetTxUnconfirmed(const uint256 &hash, CTransactionRef &txOut, uint256 &hashBlock) const
    {
...
public:
    ...
    EvalMock SetEvalMock(int currentHeight)
    {
        ...
        eval.txs[sessionTx.GetHash()] = MakeTransactionRef(sessionTx);

        CBlockIndex sessionBlock;
        sessionBlock.nHeight = 10;
        eval.blocks[sessionTx.GetHash()] = sessionBlock;

        std::vector<CTransactionRef> sessionSpends;
        sessionSpends.push_back(MakeTransactionRef(PostEvidenceTx(Dealer)));
        sessionSpends.push_back(MakeTransactionRef());  // Invalid, should be ignored
        sessionSpends.push_back(MakeTransactionRef(PostEvidenceTx(Player2)));
...
protected:
    static void SetUpTestCase() {
        // Make playerSecrets
        auto addKey = [&] (std::string k) { playerSecrets.push_back(DecodeSecret(k)); };
...
TEST_F(TestBet, testDisputeInvalidEvidence)
{
    ...
    eval.spends[ebet.SessionTx().GetHash()][1] = MakeTransactionRef(mtx);
    ...
    eval.spends[ebet.SessionTx().GetHash()][0] = MakeTransactionRef();
