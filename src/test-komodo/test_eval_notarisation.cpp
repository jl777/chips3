...
void SetEval(EvalMock &eval, CMutableTransaction &notary, Modifier modify)
{
    ...
    for (int i=0; i<notary.vin.size(); i++) {
        ...
        eval.txs[txIn.GetHash()] = MakeTransactionRef(txIn);
    }

    modify(notary);

    eval.txs[notary.GetHash()] = MakeTransactionRef(notary);
...
// inputs have been dropped, one (garbage) input remains so it parses
static const std::string rawNotaryTx = "";
CMutableTransaction notaryTx;
...
TEST(TestEvalNotarisation, testGetNotarisation)
{
    ...
    MoMProof proof;
    ASSERT_TRUE(E_UNMARSHAL(vMomProof, ss >> proof));
